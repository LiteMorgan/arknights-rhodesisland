import Document, { Html, Head, Main, NextScript } from "next/document";

export default class MyDocument extends Document {
  static async getInitialProps(ctx) {
    const initialProps = await Document.getInitialProps(ctx);
    return { ...initialProps };
  }

  render() {
    return (
      <Html lang="en">
        <Head>
          <meta charSet="utf-8" />
          {/* <link rel="alternate icon" type="image/png" href="/static/favicon.png" /> */}
          <link rel="icon" type="image/svg+xml" href="/favicon.svg" />
          <meta property="og:type" content="website" />
          {/* <meta property="og:url" content="https://nowloading.games" /> */}
          {/* <meta property="twitter:card" content="summary_large_image" /> */}
          {/* <meta property="twitter:url" content="https://nowloading.games" /> */}
          {/* <meta name="theme-color" content="#2b2b2b" /> */}

          {/* <link rel="preconnect" href="https://fonts.googleapis.com" />
          <link
            rel="preconnect"
            href="https://fonts.gstatic.com"
            crossOrigin="true"
          />
          <link
            href="https://fonts.googleapis.com/css2?family=Titillium+Web:wght@700&display=swap"
            rel="stylesheet"
          /> */}
        </Head>

        <body className="bg-gray-900 text-white w-full mx-auto px-8">
          <svg id="defs" className="hidden">
            <defs />
            <symbol id="ui-rank" viewBox="0 0 18 18">
              <path
                d="M6.97 12.216l-6.882 2.36L4.2 8.97 0 3.426l6.649 2.07L11.112 0l.24 6.96L18 9.03l-6.8 2.12-.112 6.85z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol
              id="ui-elite-0"
              viewBox="0 0 256 256"
              shapeRendering="geometricPrecision"
            >
              <path
                d="M128.006 209.744L75.285 244l17.076-40.296 13.05-8.56-75.343-49.802L11 104.022l116.994 76.508L245 104l-19.068 41.32-75.342 49.8 13.05 8.562 17.075 40.295-52.71-34.233zm0-5.962l41.74 27.126-10.11-23.858-18.129-11.893 80.482-53.203 11.183-24.224-105.178 68.758-105.16-68.754 11.183 24.224 80.482 53.203-18.129 11.893-10.11 23.858 41.746-27.13z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-elite-1" viewBox="0 0 256 256">
              <path
                d="M128.006 209.744L75.285 244l17.076-40.296 13.05-8.56-75.343-49.802L11 104.022l116.994 76.508L245 104l-19.068 41.32-75.342 49.8 13.05 8.562 17.075 40.295z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-elite-2" viewBox="0 0 256 256">
              <path
                d="M92.656 186.713L75.285 198l7.537-17.787-52.754-34.871L11 104.022l81.725 53.444 12.685-8.323-75.342-49.801L11 58.022l116.994 76.508L245 58l-19.068 41.32-75.342 49.8 12.693 8.328L245 104l-19.068 41.32-52.754 34.87 7.537 17.787-17.375-11.285-12.75 8.429 13.05 8.56 17.075 40.296-52.71-34.233L75.286 244l17.076-40.296 13.05-8.56-12.755-8.431zm22.468-14.6l12.87 8.417 12.883-8.427-12.871-8.36-12.882 8.37z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-elite-3" viewBox="0 0 256 256">
              <path
                d="M77.255 147.35l5.567-13.137-52.754-34.871L11 58.022l81.725 53.444 12.685-8.323-75.342-49.801L11 12.022 127.994 88.53 245 12l-19.068 41.32-75.342 49.8 12.693 8.328L245 58l-19.068 41.32-52.754 34.87 5.57 13.144L245 104l-19.068 41.32-52.754 34.87 7.537 17.787-17.375-11.285-12.75 8.429 13.05 8.56 17.075 40.296-52.71-34.233L75.286 244l17.076-40.296 13.05-8.56-12.755-8.431L75.285 198l7.537-17.787-52.754-34.871L11 104.022l66.255 43.328zm2.585 1.69l12.885 8.426 12.685-8.323-12.754-8.43-12.816 8.327zm35.284-22.926l12.87 8.416 12.883-8.427-12.871-8.36-12.882 8.37zm48.216 14.578l-12.75 8.429 12.693 8.327 12.883-8.426-12.826-8.33zm-48.216 31.422l12.87 8.416 12.883-8.427-12.871-8.36-12.882 8.37z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-pioneer" viewBox="0 0 256 256">
              <path
                d="M131.761 85.94L158 72l-12.968 23L88 125.288V132l-11.378-.67L49 146l8.913-15.77L20 128l40.521-2.384L62 123l62.118-33H124l4-68 3.761 63.94zm59.291 44.704l-3.02 5.356-56.49 30H132l-4 68-3.772-64.116L92 187l13-23 63-33.469V124l11.069.651L201 113l-7.062 12.526L236 128l-44.948 2.644zM36 169L226.968 67 214 90 23 192l13-23z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-sniper" viewBox="0 0 256 256">
              <path
                d="M201.736 158.928l-91.114 2.098-19.237 19.237 17.81-.715-.714 14.96-39.894 2.855-30.636 30.636-9.972-9.972 30.636-30.636 2.855-39.894 14.96-.714-.715 17.81 19.237-19.237L97.05 54.24l-9.211 5.63-4.986-4.986 31.347-19.95 66.249 16.376 8.547 8.547 11.399-4.275-4.276 11.398 8.548 8.548 16.376 66.249-19.95 31.347-4.987-4.986 5.63-9.211zm2.15-3.518l7.896-12.919-12.104-52.002-6.411-2.136-19.948 7.126-12.824 12.825-1.425-1.425-47.735 46.31-9.26.714.713-9.26 46.31-47.736-1.424-1.424 12.824-12.825 7.127-19.947-2.137-6.412-52.002-12.103-12.919 7.896L97.8 156.753l-24.224 24.224v1.425H75l24.224-24.224 104.662-2.768zM162.524 28H228v65.493l-11.08-54.41L162.523 28zM80.93 134.804l2.519-57.432 2.518 57.432H80.93zm39.286 35.265l57.418 2.52-57.418 2.518v-5.038z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-medic" viewBox="0 0 256 256">
              <path
                d="M113.985 120.127V26h28.03v94.127L223 167.191l-14.015 24.433L128 144.56l-80.985 47.064L33 167.19l80.985-47.064zM128 161.483l14.015 9.069V230h-28.03v-59.448L128 161.483zm-29.042-50.632l-14.81 7.682L33 88.81l14.015-24.433L98.163 94.1l.795 16.751zm58.084 0l.795-16.75 51.148-29.725L223 88.81l-51.148 29.724-14.81-7.682zM69 35.09h30v10.1H79v20.197H69V35.09zm118 0v30.297h-10V45.188h-20V35.09h30zM69 220.911v-30.297h10v20.198h20v10.099H69zm118 0h-30v-10.1h20v-20.197h10v30.297zM28 128l21-21.208 8 8.08L44 128l13 13.129-8 8.079L28 128zm200 0l-21 21.208-8-8.08L212 128l-13-13.129 8-8.079L228 128z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-caster" viewBox="0 0 256 256">
              <path
                d="M222.154 232l-8.56-7.148-.713-2.144-55.636-55.753-10.7-6.433-11.412-11.436 2.14-2.144L84.49 94.048V66.887l12.125 12.15v11.437l48.504 48.605 2.14-2.144 11.412 11.436 6.42 10.722 55.636 55.752 2.14.715 7.133 8.577-7.846 7.863zM79.497 91.904H45.259L26.713 73.32V37.58l10.7-10.72 7.132 7.147-7.846 7.862v27.162l12.84 12.866h29.958v10.007zM42.406 43.299h21.398l11.413 11.436V76.18H53.818L42.406 64.742V43.3zM26 85.471l29.245 29.306-22.469-6.79L26 181.25V85.47zM58.811 24h95.58L81.28 30.79l6.776 22.516L58.81 24z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-warrior" viewBox="0 0 256 256">
              <path
                d="M63.268 63.268v32.843H50.417v-34.27L24 35.424 35.424 24 61.84 50.417h34.27v12.851H63.268zm159.93 159.93l-44.98-22.133-7.739-15.109L70.523 86l.208-13.058 88.532 88.532 2.211-2.21L72.942 70.73 86 70.523l99.956 99.956 15.109 7.739 22.133 44.98zm8.802-8.363l-30.291-75.728 23.223 25.243L232 97.709v117.126zM214.835 232H97.709l66.64-7.068-25.242-23.223L214.835 232z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-tank" viewBox="0 0 256 256">
              <path
                d="M128 33l25.269 17h25.269v13l25.268 11-6.064 98L128 226l-69.742-54-6.064-98 25.268-11V50h25.27L128 33zm22.237 27L128 46l-22.237 14H85.548v11L62.301 81l5.054 86L128 214l60.645-47 5.054-86-23.247-10V60h-20.215zm-26.28 61h8.086V56l16.172 11h18.194l-5.054 81L128 174l-33.355-26-5.054-81h18.194l16.172-11v65zM128 180l38.409-30 4.043-70 14.15 6-4.043 76L128 203l-52.56-41-4.042-76 14.15-6 4.043 70L128 180zM59.269 62L34 30l32.344 24-7.075 8zm0 126l7.075 8L34 220l25.269-32zM196.73 62l-7.075-8L222 30l-25.269 32zm0 126L222 220l-32.344-24 7.075-8z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-support" viewBox="0 0 256 256">
              <path
                d="M193 204h11V52h-50v16h34v105.858l-86-86V52H86v34H52v118h30v-16H68v-86h19.858l86 86H136v-28.858l-16-16V220H36V36h84v60.758l16 16V36h84v184h-84v-16h57zM52 52v26h26V52H52z"
                fillRule="nonzero"
              />
            </symbol>
            <symbol id="ui-special" viewBox="0 0 256 256">
              <path
                d="M196.297 187l11.64 23.351L228 228l-21.675-11H49.675L28 228l18.308-16.105L69.186 166H86.27L69.38 200h116.256l-6.458-13h17.12zm-8.973-18H133.42l8.867-18h19.005l-33.784-68-23.35 47h27.291l-8.867 18H78.158l46.6-93.483L127.507 28l2.75 26.517L187.323 169z"
                fillRule="nonzero"
              />
            </symbol>
          </svg>

          <Main />
          <NextScript />
        </body>
      </Html>
    );
  }
}
